...
int channelCheck1(const int clientSocket,Channel *channels,const Client *clients, std::string channel, int channel_index2, const int i)
{
...
void channelCheck2(const int clientSocket,Channel *channels,const Client *clients, int channel_index2, const int i)
{
	if (!searchIfExist(channels[channel_index2].users_sockets, clients[i].socket))
	{
...
void channelCheck3(const int clientSocket,Channel *channels,const Client *clients, std::string channel, int channel_index2, const int i)
{
		if (searchIfExist(channels[channel_index2].users_sockets, clients[i].socket))
		{
			std::string channelFullPrompt = "User Already In This Channel\n";
...
int channelExist(const int clientSocket,Channel *channels,const Client *clients, std::string channel, const int i)
{
	...
	if (channels[channel_index2].invite_only == 1)
		channelCheck1(clientSocket, channels, clients, channel, channel_index2, i);
	if (channels[channel_index2].users_sockets.size() < channels[channel_index2].limit || channels[channel_index2].limit_mode == 0)
		channelCheck2(clientSocket, channels, clients, channel_index2, i);
	else
		channelCheck3(clientSocket, channels, clients, channel, channel_index2, i);
	return (0);
}

void checkbytesRead(char *TopicBuffer, ssize_t bytesRead, Channel *channels, int *channel_index)
{
...
void checkbytesRead3(const int clientSocket,Channel *channels,const Client *clients, ssize_t bytesRead3, std::string channel, char *limitBuffer, const int i, int *channel_index)
{
...
void checkbytesRead4(const int clientSocket, Channel *channels, const Client *clients, std::string channel, char *indexBuffer, const int i, ssize_t bytesRead, ssize_t bytesRead4, int *channel_index)
{
...
void channelNotExist(const int clientSocket,Channel *channels,const Client *clients, std::string channel, const int i, int *channel_index)
{
	...
	try{
		...
		if (bytesRead > 0)
			checkbytesRead(TopicBuffer, bytesRead, channels, channel_index);
		...
		ssize_t bytesRead3 = read(clientSocket, limitBuffer, sizeof(limitBuffer));
			checkbytesRead3(clientSocket, channels, clients, bytesRead3, channel, limitBuffer, i, channel_index);
		...
		if (bytesRead4 > 0)
			checkbytesRead4(clientSocket, channels, clients, channel, indexBuffer, i, bytesRead, bytesRead4, channel_index);
	}
